    ...
    <div className="min-h-dvh bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[232px] border-r border-border bg-card lg:flex lg:flex-col">
        ...
        <nav className="flex-1 px-2.5 py-4" aria-label={t("主要导航")}>
          <div className="px-2 pb-2 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{t("工作区")}</div>
      ...
      <div className="lg:pl-[232px]">
        ...
        <main className="pb-16 lg:pb-0"><Outlet /></main>
      </div>

      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-2 border-t border-border bg-card px-2 pb-[max(.4rem,env(safe-area-inset-bottom))] pt-1.5 lg:hidden" aria-label={t("移动端导航")}>
        {navigation.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            end={item.end}
            className={({ isActive }) => cn("flex items-center justify-center rounded-md py-2 text-[11px] text-muted-foreground", isActive && "font-medium text-foreground")}
          >
            {t(item.label)}
          </NavLink>
        ))}

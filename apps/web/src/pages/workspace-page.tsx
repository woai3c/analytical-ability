...
import { emptyGoalInput, goalInputSchema, taskTypes, type GoalInput, type LlmAnalysis } from "@clarity/domain";
  ...
  return (
    <div className="mx-auto w-full max-w-[1480px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <Button variant="outline" onClick={resetGoal}>{t("新建目标")}</Button>
      </div>

      <ol className="mb-6 grid grid-cols-4 gap-1 rounded-lg border border-border bg-card p-1.5 sm:gap-2" aria-label={t("分析进度")}>
        {steps.map((item) => (
          <li key={item.id}>
            <button
              type="button"
        ...
        <section>
          {step === 1 ? (
            <Card className="overflow-hidden">
              <CardContent className="p-6 sm:p-10">
                <div className="mb-5 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{t("新建分析")}</div>
                <h2 className="max-w-3xl font-serif text-3xl leading-tight tracking-tight sm:text-[34px]">{t("你现在最想完成的目标是什么？")}</h2>
                <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">{t("先用自己的话描述。系统会区分事实、假设和未知项，告诉你还缺什么数据，而不是直接生成一篇看似正确的答案。")}</p>
                <Textarea
                  className="mt-7 min-h-40 bg-background text-base leading-7 sm:text-lg"
                  value={goal.rawGoal}
                  onChange={(event) => update("rawGoal", event.target.value)}
                  placeholder={t("例如：我想在六个月内转向 AI 产品经理，但不知道需要补哪些能力、收集什么岗位数据，也不知道该从哪一步开始。")}
                  autoFocus
                />
                <div className="mt-4 flex flex-wrap gap-2">
                  <span className="self-center text-xs text-muted-foreground">{t("加载完整示例：")}</span>
                  {examples.map((example) => (
                    <Button key={example.label} type="button" variant="outline" size="sm" onClick={() => applyExample(example)}>{example.label}</Button>
                  ))}
                </div>
                <div className="mt-8 flex justify-end">
                  <Button size="lg" disabled={goal.rawGoal.trim().length < 4} onClick={() => setStep(2)}>{t("开始拆解")}</Button>
                </div>
              </CardContent>
            </Card>
          ) : null}
          ...
          {step === 2 ? (
            <Card>
              <CardHeader className="border-b border-border p-6 sm:px-8">
                ...
                <CardDescription>{t("不知道的字段可以先留空；右侧会解释为什么需要它，而不会替你捏造。")}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-7 p-6 sm:p-8">
                <Field label={t("原始目标")} hint={t("必填")} htmlFor="raw-goal">
                  <Textarea id="raw-goal" value={goal.rawGoal} onChange={(event) => update("rawGoal", event.target.value)} />
                </Field>
                <div className="grid gap-6 md:grid-cols-2">
                  <Field label={t("当前起点")} hint={t("基线")} htmlFor="current-state">
                    <Textarea id="current-state" value={goal.currentState} onChange={(event) => update("currentState", event.target.value)} placeholder={t("现在已经有什么、做到什么程度？")} />
                  </Field>
                  <Field label={t("期望结果")} hint={t("可观察结果")} htmlFor="desired-outcome">
                    <Textarea id="desired-outcome" value={goal.desiredOutcome} onChange={(event) => update("desiredOutcome", event.target.value)} placeholder={t("完成后具体会发生什么？")} />
                  </Field>
                </div>
                <div className="grid gap-6 md:grid-cols-2">
                  <Field label={t("成功标准")} hint={t("指标或证据")} htmlFor="success-metric">
                    <Input id="success-metric" value={goal.successMetric} onChange={(event) => update("successMetric", event.target.value)} placeholder={t("例如：连续四周达到 45%")} />
                  </Field>
                  <Field label={t("期限")} hint={t("决定计划强度")} htmlFor="deadline">
                    <Input id="deadline" type="date" value={goal.deadline} onChange={(event) => update("deadline", event.target.value)} />
                  </Field>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {taskTypes.map((type) => (
                      <button
                        ...
                        className={cn(
                          ...
                          (goal.preferredTaskType ?? analysis?.taskType) === type && "border-primary bg-secondary font-medium text-secondary-foreground",
                        )}
                    ...
                    {goal.preferredTaskType ? (
                      <button type="button" className="px-2 text-xs text-muted-foreground underline" onClick={() => update("preferredTaskType", null)}>{t("恢复自动判断")}</button>
                    ) : null}
                  </div>
                </div>
                <div className="grid gap-6 md:grid-cols-2">
                  <TagEditor id="constraints" label={t("限制条件")} hint={t("硬边界与偏好")} placeholder={t("例如：每周最多投入 10 小时")} values={goal.constraints} onChange={(values) => update("constraints", values)} />
                  <TagEditor id="known-facts" label={t("已知事实")} hint={t("需要来源")} placeholder={t("例如：过去四周平均完成率为 28%")} values={goal.knownFacts} onChange={(values) => update("knownFacts", values)} />
                </div>
                ...
                <CardHeader className="border-b border-border p-6 sm:px-8">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <CardDescription>{t("每一项都说明收集原因、字段和做法。必要数据不等于所有可能有用的数据。")}</CardDescription>
                    </div>
                  ...
                  {analysis.dataNeeds.map((need, index) => (
                    <article key={need.id} className="rounded-lg border border-border bg-background p-5">
                      ...
                      <div className="mt-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">{need.fields.map((field) => <Badge key={field} variant="outline">{field}</Badge>)}</div>
                      </div>
                      <div className="mt-4 rounded-md bg-muted p-3 text-xs leading-5 text-muted-foreground"><span className="font-medium text-foreground">{t("怎么做：")}</span>{need.collectionMethod}</div>
                    </article>
                  ))}
                </CardContent>
              </Card>
              <div className="flex justify-between">
                <Button variant="ghost" onClick={() => setStep(2)}>{t("修改条件")}</Button>
          ...
          {step === 4 && analysis ? (
            <div className="space-y-5">
                    ...
                    {analysis.actionSteps.map((action, index) => (
                      <li key={action.id} className="grid grid-cols-[28px_1fr] gap-3">
                        ...
                        <div className="rounded-lg border border-border bg-background p-4 sm:p-5">
                          ...
                          <div className="mt-3 rounded-md bg-muted px-3 py-2 text-xs text-muted-foreground"><strong className="font-medium text-foreground">{t("完成标准：")}</strong>{action.doneWhen}</div>
                        </div>
                      </li>
                    ))}
                  </ol>
              ...
              <div className="flex justify-between">
                <Button variant="ghost" onClick={() => setStep(3)}>{t("返回数据计划")}</Button>
        ...
        <aside className="space-y-4 xl:sticky xl:top-[68px]">
              ...
              {analysis ? (
                <>
                  <div className="mb-2 flex items-end justify-between">
                    ...
                    <span className="text-2xl font-semibold">{analysis.completeness}<span className="text-sm text-muted-foreground">%</span></span>
                  </div>
                  <Progress value={analysis.completeness} />
                  <p className="mt-4 text-sm leading-6 text-muted-foreground">{analysis.summary}</p>
                </>
              ) : <p className="py-6 text-center text-sm leading-6 text-muted-foreground">{t("输入目标后，这里会显示任务类型、缺口和风险。")}</p>}
            </CardContent>
          </Card>
          ...
          {analysis ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t("规则负责稳定的流程骨架，智能补充只发现目标上下文中的隐含假设和检索词。")}</CardDescription>
              </CardHeader>
              <CardContent>
                {!llmResult ? (
                  <Button className="w-full" variant="outline" disabled={llmLoading} onClick={runSemanticAssist}>{t(llmLoading ? "正在分析" : "补充语义分析")}</Button>
                ) : (
                  <div className="space-y-4 text-xs leading-5">
                    <div><div className="font-medium text-foreground">{t("目标重述")}</div><p className="mt-1 text-muted-foreground">{llmResult.goalRestatement}</p></div>
                    {llmResult.assumptions.length ? (
                      <div><div className="font-medium text-foreground">{t("隐含假设")}</div><ul className="mt-1 list-disc space-y-1 pl-4 text-muted-foreground">{llmResult.assumptions.map((item) => <li key={item}>{item}</li>)}</ul></div>
                    ) : null}
                    <div><div className="font-medium text-foreground">{t("建议下一步")}</div><p className="mt-1 text-muted-foreground">{llmResult.suggestedNextStep}</p></div>
                    <Button className="w-full" size="sm" variant="ghost" onClick={runSemanticAssist}>{t("重新生成")}</Button>
                  </div>
                )}
          ...
          {analysis ? (
            <Card className="border-[color-mix(in_oklch,var(--warning)_38%,var(--border))]">
              <CardHeader><CardTitle>{t("分析边界")}</CardTitle></CardHeader>
              <CardContent><ul className="list-disc space-y-2 pl-4 text-xs leading-5 text-muted-foreground">{analysis.cautions.map((caution) => <li key={caution}>{caution}</li>)}</ul></CardContent>
            </Card>
          ) : null}
        </aside>
      </div>

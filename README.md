# Clarity

> AI doesn't think for you. It trains you to think.

[简体中文](./README.zh-CN.md) · [Live Demo](https://analytical-ability-web.vercel.app/)

Clarity is an open-source analytical thinking trainer. Learn when and how to apply methods such as 5 Whys, Fishbone Diagram, SWOT, and MCDA through realistic AI-generated scenarios and immediate feedback.

## Features

- **Method Library** — 12 analysis methods with full introductions, step-by-step guides, animated walkthroughs, and worked examples
- **Scenario Training** — AI-generated real-world scenarios; 5-step guided analysis training with targeted feedback and built-in hints
- **Progress Tracking** — Practice history organized by scenario type and method, with trend comparison
- **Pure Frontend** — No backend required; deploy to Vercel / Netlify / GitHub Pages
- **Data Ownership** — Practice data stored in browser localStorage with export / import

## Get Started

> Pure frontend app. No server or database to configure.

### Local Development

```bash
git clone https://github.com/woai3c/clarity.git
cd clarity
pnpm install
pnpm dev
```

Open `http://localhost:5173`, go to **Settings** and configure your API Key.

### Deploy

Clarity is a static SPA deployable to any hosting platform:

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/woai3c/clarity)

#### Vercel

1. Click the button above or go to [vercel.com/new](https://vercel.com/new) and import this repository
2. Keep default settings (Framework Preset will auto-detect as Vite)
3. No environment variables needed — click **Deploy**
4. After deployment, open the site and go to **Settings** to configure your AI API Key

#### Other Platforms

```bash
pnpm build     # Output: dist/
```

Deploy the `dist` directory to Netlify / GitHub Pages / Cloudflare Pages or any static hosting.

## Supported AI Providers

| Provider          | Default Model     |
| ----------------- | ----------------- |
| DeepSeek          | deepseek-v4-flash |
| Anthropic         | claude-sonnet-5   |
| OpenAI            | gpt-5.6-sol       |
| Google Gemini     | gemini-3.5-flash  |
| xAI (Grok)        | grok-4.3          |
| Alibaba (Qwen)    | qwen3.7-max       |
| Zhipu (GLM)       | glm-5.2           |
| Moonshot (Kimi)   | kimi-k3           |
| OpenAI Compatible | Custom            |

Select a provider and enter your API Key in the Settings page. Keys are stored only in your browser.

## Tech Stack

| Layer           | Technology                         |
| --------------- | ---------------------------------- |
| Framework       | React 19 + Vite 8                  |
| Styling         | Tailwind CSS 4                     |
| AI              | Vercel AI SDK (`ai` + `@ai-sdk/*`) |
| Language        | TypeScript                         |
| Package Manager | pnpm                               |

## Project Structure

```
src/
  data/           Data contracts, method registry & routing rules
  lib/            LLM calls, API wrappers
  components/     UI components
  pages/          Page components
  providers/      Context Providers
  styles/         Theme variables (Light / Dark)
public/           Static assets
index.html        Entry HTML
vite.config.ts    Vite config
```

## Development

```bash
pnpm dev        # Start dev server
pnpm build      # Production build
pnpm lint       # ESLint
pnpm typecheck  # TypeScript type checking
pnpm test       # Run tests
pnpm ci         # lint + format + typecheck + test + build
```

## FAQ

**Q: Do I need my own API Key?**

Yes. Clarity is a pure frontend app — AI calls go directly from your browser. Get an API Key from DeepSeek / OpenAI / Google and configure it in Settings.

**Q: Where is my data stored?**

All practice data is stored in your browser's localStorage. You can export it as a JSON file for backup or migration in Settings.

**Q: Can I use it offline?**

The Method Library pages work offline. Scenario Training requires a network connection to call AI models.

## License

[MIT](./LICENSE)
